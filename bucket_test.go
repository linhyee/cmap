...
func TestBucketPut(t *testing.T) {
	...
	for _, p := range testCases {
		...
		if ok {
			t.Fatalf("Couldn't put repeat pair to the bucket! (pair:%#v)", p)
		}
		count++
		if b.Size() != count {
...
func TestBucketClearParallel(t *testing.T) {
	number := 1000
	testCases := genTestingPairs(number)
	b := newBucket()
	lock := new(sync.Mutex)
	...
	t.Logf("Now bucket size:%d", b.Size())
	if b.Size() > 0 {
...
var testCasesNumberForBucketTest = 200000
var testCasesForBucketTest = genNoRepetitiveTestingPairs(testCasesNumberForBucketTest)
var testCases1ForBucketTest = testCasesForBucketTest[:testCasesNumberForBucketTest/2]
var testCases2ForBucketTest = testCasesForBucketTest[testCasesNumberForBucketTest/2:]

func TestBucketAllParallel(t *testing.T) {
	testCase1 := testCases1ForBucketTest
	testCase2 := testCases2ForBucketTest
	b := newBucket()
	lock := new(sync.Mutex)
	t.Run("All in parallel", func(t *testing.T) {
		t.Run("Put1", func(t *testing.T) {
			t.Parallel()
			for _, p := range testCase1 {
				...
				if err != nil {
					t.Fatalf("An error when putting a pair to the bucket: %s (pair: %#v)", err, p)
				}
			}
		})
		t.Run("Put2", func(t *testing.T) {
			t.Parallel()
			for _, p := range testCase2 {
				existingPair := b.Get(p.Key())
				if existingPair != nil {
					b.Delete(p.Key(), lock)
				}
...
// genNoRepetitiveTestingPairs 用于生成测试的无重复的键-元素对的切片
